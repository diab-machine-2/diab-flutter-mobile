...
import 'package:medical/bloc/HbA1C/HbA1C_bloc.dart';
...
class FoodChartState extends State<FoodChart>
  ...
  buildChart(FoodDietModel model) {
    ...
    double minY = data
        .map<double>((e) =>
            (e.details.map<double>((element) => element.value).reduce(min)))
        .reduce(min);
    minY = (minY * (data.length == 1 ? 0.5 : 0.8)).roundToDouble();
    double maxY = data.map<double>((e) => (e.value)).reduce(max);
