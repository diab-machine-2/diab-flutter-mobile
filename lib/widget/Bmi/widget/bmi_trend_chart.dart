...
import 'package:medical/bloc/weight/weight_bloc.dart';
import 'package:medical/modal/bmi/weight_trend.dart';
import 'package:medical/modal/bmi/weight_trend_item.dart';
import 'package:medical/modal/glucose/glucose_data_trend.dart';
import 'package:medical/modal/glucose/glucose_trend.dart';
...
class BmiTrendChart extends StatefulWidget {
  ...
  @override
  BmiTrendChartState createState() => BmiTrendChartState();
}
...
class BmiTrendChartState extends State<BmiTrendChart>
    ...
    return BlocProvider<WeightBloc>(
        ...
        child: BlocBuilder<WeightBloc, WeightState>(
          ...
          if (state is WeightInitial) {
            BlocProvider.of<WeightBloc>(context).add(FetchTrendWeight(
                currentDateTime:
                    (DateTime.now().millisecondsSinceEpoch ~/ 1000).toString(),
                periodFilterType: periodFilterType.toString(),
                page: '1'));
  ...
  buildChart(TrendWeightModel model) {
    ...
    double minY = trends.map<double>((e) => e.value).reduce(min);
    minY = (minY * (trends.length == 1 ? 0.5 : 0.8)).roundToDouble();
    double maxY = trends.map<double>((e) => e.value).reduce(max);
