...
mixin AddBmiMixin on Widget {
  int findIndexInRanges({
    required AddBmiCubit cubit,
  }) {
    List<double> ranges = cubit.rangeValue;
    if (cubit.selectedWeight == null) return -1;

    for (int i = 0; i < ranges.length - 1; i++) {
      if (cubit.selectedWeight >= ranges[i] &&
          cubit.selectedWeight <= ranges[i + 1]) {
        return i;
      }
    }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Text(
                            R.string.weight_at.tr() +
                                ' ${cubit.weightRanges != null ? cubit.weightRanges![indexRange].name : ""}',
                          ...
                          children: [
                            Expanded(
                              child: GestureDetector(
                                  onTap: () {
                                    cubit.controllerWeight.selection =
                                        TextSelection(
                                            baseOffset: 0,
                                            extentOffset: cubit
                                                .controllerWeight.text.length);
  ...
  showDialogDelete(BuildContext context, AddBmiCubit cubit) {
    showDialog(
      context: context,
      builder: (context) {
        return Container(
  ...
  showDialogSave(
    ...
    final note = cubit.controllerNote.text;
    if (cubit.model != null) {
      final date =
          DateTime.fromMillisecondsSinceEpoch(cubit.model!.date! * 1000);
      if (cubit.selectedWeight == 0 &&
          cubit.selectedHeight == 0 &&
          cubit.selectedHip == 0 &&
          note.isEmpty &&
          cubit.files.length == cubit.model!.images.length &&
          cubit.removeIDs.length == 0 &&
          date.millisecondsSinceEpoch ==
              cubit.selectedDate.millisecondsSinceEpoch) {
        Navigator.pop(context);
        return;
      }
    } else if (cubit.selectedWeight == 0 &&
        cubit.selectedHip == 0 &&
        note.isEmpty &&
        note.isEmpty &&
        cubit.files.length == 0) {
      Navigator.pop(context);
      return;
    }
