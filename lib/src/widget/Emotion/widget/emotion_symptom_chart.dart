...
class EmotionSymptomChartState extends State<EmotionSymptomChart>
  ...
  Future<bool> _refresh() async {
    BlocProvider.of<EmotionBloc>(currentContext).add(FetchSymptomStatistic(
      currentDateTime:
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider<EmotionBloc>(
        ...
        child: BlocBuilder<EmotionBloc, EmotionState>(
          ...
          if (state is SymptomStatisticLoaded) {
            model = state.symptoms;
          }
          return model == null
              ? SizedBox()
              : model.length == 0
