...
class BranchioLinkConfig {
  ...
  static final BranchioLinkConfig instance = BranchioLinkConfig._privateConstructor();
  ...
  void setUpHandleDeepLink() {
    _subLink = FlutterBranchSdk.initSession().listen((data) async {
      ...
      if (data['+clicked_branch_link'] == true && data.containsKey("\$course}")) {
        processBookingCourseLink(data['\$course'] as String);
        return;
      }
  ...
  void processBookingCourseLink(String courseId) {
    if (AppSettings.userInfo == null) {
      _courseId = courseId;
    } else {
      // Navigate
      navigatorKey.currentState?.pushNamed(NavigatorName.calendar_booking, arguments: courseId);
    }
  }
