...
public class DeviceAdapter extends BaseAdapter {
    ...
    public ArrayList<ExtendedDevice> getList() {
        return _listItems;
    }

    public ExtendedDevice getItem(String address) {
       ArrayList<ExtendedDevice> list = _listItems.stream().filter(item -> address.equals(item.device.getAddress())).collect(Collectors.toCollection(ArrayList::new));
       if (list.size() != 0) {
return list.get(0);
       } else {
           return null;
    ...
    @Override
    public ExtendedDevice getItem(int position) {
